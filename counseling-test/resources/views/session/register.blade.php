          ...
          <div class="card z-index-0">
            ...
            <div class="card-body">
              <form role="form text-left" method="POST" action="/register">
                @csrf
                <div class="mb-3">
                  <input type="text" class="form-control" placeholder="Name" name="namaUser" id="namaUser" aria-label="Your Name" aria-describedby="name">
                  @error('namaUser')
                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
                </div>
                <div class="mb-3">
                  <input type="text" class="form-control" placeholder="Username" name="username" id="username" aria-label="username" aria-describedby="email-addon">
                  @error('username')
                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                  @enderror
                </div>
                <div class="mb-3">
                    <select type="text" class="form-control" placeholder="Role" name="role" id="role" aria-label="role">
